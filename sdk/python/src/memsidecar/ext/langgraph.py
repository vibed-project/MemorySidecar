"""LangGraph checkpointer backed by memsidecar.

``MemSidecarSaver`` implements LangGraph's :class:`BaseCheckpointSaver` on top of
the sidecar's **kv** block, so a graph gets durable, multi-tenant thread state by
pointing at a running memsidecar — no bespoke persistence glue::
...
Requires ``langgraph-checkpoint`` (``pip install memsidecar[langgraph]``).
"""
...
class MemSidecarSaver(BaseCheckpointSaver):
    """A LangGraph checkpointer whose storage is a memsidecar kv namespace.
