"""CrewAI memory storage backend backed by memsidecar.

``MemSidecarStorage`` implements CrewAI's ``StorageBackend`` protocol (the
pluggable storage under CrewAI 1.x's unified ``Memory``), so a crew's memories
live on a running memsidecar instead of a local vector file::
    ...
    client = MemSidecar("127.0.0.1:7777", token=TOKEN)
    ...
    set_memory_storage_factory(lambda spec: MemSidecarStorage(client))
...
vectors as-is and searches by vector — the memsidecar **semantic** namespace's
...
Requires ``crewai`` (``pip install memsidecar[crewai]``).
"""
...
class MemSidecarStorage:
    """A CrewAI ``StorageBackend`` whose storage is a memsidecar instance.

    Args:
        client: A connected :class:`memsidecar.MemSidecar`.
