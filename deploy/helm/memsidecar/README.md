# memsidecar Helm chart

Installs the memsidecar gRPC server into Kubernetes.
...
# values.example.yaml
image:
  repository: ghcr.io/your-org/memsidecar
  tag: v0.1.0
...
tls:
  enabled: true
  existingSecret: memsidecar-server-tls
```
...
|---|---|---|
| `Deployment` | yes | runs the memsidecar binary |
| `Service` | yes | exposes ports `grpc/http/metrics` |
| `ConfigMap` | yes | mounts `config.yaml` at `/etc/memsidecar/config.yaml` |
...
port. Probing the `memsidecar.kv.v1.KV` service ensures the wiring is fully
