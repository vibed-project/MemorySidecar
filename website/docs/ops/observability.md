...
- `rpc.method`, `rpc.system.name`, `rpc.response.status_code` — added by
  `otelgrpc`.
- `mindd.method`, `mindd.tenant`, `mindd.agent` — added by
...
mindD registers Go runtime + process collectors out of the box plus
...
On top of the raw gRPC histogram, mindD emits its own memory-aware
...
- `mindd.embedder.cache.hits` / `.misses` — embedding-cache effectiveness
...
- `mindd.namespace.items` — an observable **gauge** of the live item count
...
- `mindd.eviction.total` — a **counter** of items dropped from a namespace,
  ...
  `consolidation` is reserved. `rate(mindd_eviction_total[5m])` is your
  eviction churn.
