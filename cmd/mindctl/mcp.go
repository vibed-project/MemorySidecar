...
// protocol translation: every tool proxies to the mindd gRPC API under a
...
func cmdMCP(args []string) int {
	...
	addr := fs.String("addr", mcpEnvOr("MINDD_ADDR", "127.0.0.1:7777"), "server address host:port")
	...
	if *token == "" {
		fmt.Fprintln(os.Stderr, "mcp: no token: pass --token or set $MINDD_TOKEN")
		return 2
	}
