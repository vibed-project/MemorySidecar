...
// common case (`mindctl kv get ns key`) needs no flags.
func connFlags(fs *flag.FlagSet) func() (*grpc.ClientConn, error) {
	addr := fs.String("addr", envOr("MINDD_ADDR", "127.0.0.1:7777"), "server address host:port")
	token := fs.String("token", os.Getenv("MINDD_TOKEN"), "capability token (defaults to $MINDD_TOKEN)")
	useTLS := fs.Bool("tls", false, "dial over TLS")
	return func() (*grpc.ClientConn, error) {
		if *token == "" {
			return nil, fmt.Errorf("no token: pass --token or set $MINDD_TOKEN")
		}
...
func cmdKV(args []string) int {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: mindctl kv (get|put) ...")
		return 2
	}
	...
	case "get":
		...
		if fs.NArg() != 2 {
			fmt.Fprintln(os.Stderr, "usage: mindctl kv get <namespace> <key>")
			return 2
		}
		conn, err := dial()
		if err != nil {
	...
	case "put":
		...
		if fs.NArg() != 3 {
			fmt.Fprintln(os.Stderr, "usage: mindctl kv put <namespace> <key> <value> [--ttl 5m]")
			return 2
		}
		conn, err := dial()
		if err != nil {
...
func cmdSemantic(args []string) int {
	if len(args) < 1 || args[0] != "search" {
		fmt.Fprintln(os.Stderr, "usage: mindctl semantic search <namespace> <query> [--top-k 10] [--filter k=v,...]")
		return 2
	}
...
func cmdEpisodic(args []string) int {
	if len(args) < 1 || args[0] != "tail" {
		fmt.Fprintln(os.Stderr, "usage: mindctl episodic tail <namespace> [--historical] [--after-cursor N]")
		return 2
	}
...
func cmdGraph(args []string) int {
	if len(args) < 1 || args[0] != "neighbors" {
		fmt.Fprintln(os.Stderr, "usage: mindctl graph neighbors <namespace> <node-id> [--limit N] [--edge-types a,b]")
		return 2
	}
...
func cmdNS(args []string) int {
	if len(args) < 1 || args[0] != "ls" {
		fmt.Fprintln(os.Stderr, "usage: mindctl ns ls")
		return 2
	}
